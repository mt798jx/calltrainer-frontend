...
function ScenariosPageContent() {
  ...
  >([]); // História dialógu
  const [inputMessage, setInputMessage] = useState(""); // Aktuálna správa operátora
  ...
  /**
   * Odošle správu operátora do AI.
   * AI vygeneruje odpoveď volajúceho a obe správy sa pridajú do dialógu.
   */
  const handleSendMessage = async () => {
    ...
    // Okamžite pridaj správu operátora
    const operatorMsg = {
      role: "operator",
      message: userMessage,
      timestamp: new Date().toISOString(),
    };
    setDialogue((prev) => [...prev, operatorMsg]);

    // Pridaj načítavaciu indikáciu (tri bodky)
    const loadingMsg = {
      role: "caller",
      message: "...",
      timestamp: new Date().toISOString(),
    };
    setDialogue((prev) => [...prev, loadingMsg]);

    try {
      const result = await chatSimulation.mutateAsync({
        session_id: sessionId,
        message: userMessage,
      });

      // Odstráň načítavaciu indikáciu a pridaj skutočnú odpoveď AI
      setDialogue((prev) => {
        // Odstráň poslednú správu (načítavacie bodky)
        const withoutLoading = prev.slice(0, -1);
        // Pridaj len odpoveď AI (druhá položka v dialogue_append)
        return [...withoutLoading, result.dialogue_append[1]];
      });
    } catch (error) {
      console.error("Failed to send message:", error);
      // Odstráň načítavacie bodky pri chybe
  ...
  const handleEndCall = async () => {
    ...
    try {
      const result = await endSimulation.mutateAsync(sessionId);

      toaster.push(
        <Message showIcon type="success">
          {`Hovor ukončený!\nSkóre: ${result.score}%\nStatus: ${result.status === "completed" ? "Úspešný" : "Neúspešný"}`}
        </Message>,
        { placement: "topEnd" },
      );

      router.push("/operatorBoard/tasks");
    } catch (error) {
            ...
            <div className="scenarios-conversation-large">
              ...
              >
                <div style={{ position: "relative" }}>
                  <textarea
                    id="scenarios-transcript"
                    placeholder="Napíšte vašu odpoveď volajúcemu..."
                    value={inputMessage}
                  ...
                  <button
                    ...
                    disabled={!inputMessage.trim() || chatSimulation.isPending}
                    style={{
                      position: "absolute",
                      right: "8px",
                      top: "50%",
